...
var Triangulate = {
  ...
  quad: function(data, a, b, c, d) {
    this.triangle(data, a, b, c);
    this.triangle(data, c, d, a);
  },

  triangle: function(data, a, b, c) {
    var n = normal(a, b, c);
  ...
  circle: function(data, center, radius, Z) {
    Z = Z || 0;
    var u, v;
...
//      return;
  ...
  cube: function(data, sizeX, sizeY, sizeZ, X, Y, Z) {
    X = X || 0;
    Y = Y || 0;
    Z = Z || 0;
    ...
    this.quad(data, b, a, d, c);
    this.quad(data, A, B, C, D);
    this.quad(data, a, b, B, A);
    this.quad(data, b, c, C, B);
    this.quad(data, c, d, D, C);
    this.quad(data, d, a, A, D);
  },

  cylinder: function(data, center, radius1, radius2, height, Z) {
    Z = Z || 0;
    var
      currAngle, nextAngle,
      currSin, currCos,
  ...
  dome: function(data, center, radius, height, Z) {
    Z = Z || 0;
    var
      currAngle, nextAngle,
      currSin, currCos,
    ...
    for (var i = 0; i < num; i++) {
      ...
      this.cylinder(data, center, nextRadius, currRadius, nextHeight, nextZ);
    }
  },

  // TODO
  sphere: function(data, center, radius, height, Z) {
    Z = Z || 0;
    return this.cylinder(data, center, radius, radius, height, Z);
  },

  pyramid: function(data, polygon, center, height, Z) {
    Z = Z || 0;
    polygon = polygon[0];
  ...
  extrusion: function(data, polygon, height, Z, tx) {
    Z = Z || 0;
    var
      ring, last, a, b,
      L,
